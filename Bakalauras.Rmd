...
pradiniai_duomenys <- read_sas("C:\\Users\\vytau\\Desktop\\Bakalauras\\Destytojo atsiusti\\1_mail_visi_csm.sas7bdat")
...
Vizualizaciju_Aplankas <-
  "C:\\Users\\vytau\\Desktop\\Bakalauras\\Analize\\Vizualizacijos"
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Limfmazgių_būklė,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Pašalintų_kraštų_būklė,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Limfmazgių_skaičius,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
...
###############################################################################
# Shapiro-Wilks test'as Amžiaus kintamąjam
shapiro.test(homogeniai_duomenys$Amzius)
# Kolmogorov-Smirnov test'as Amžiaus kintamąjam
ks.test(
  homogeniai_duomenys$Amzius,
  "pnorm",
  mean = mean(homogeniai_duomenys$Amzius),
  sd = sd(homogeniai_duomenys$Amzius)
)
normalumas_amzius <-
  ggqqplot(homogeniai_duomenys$Amzius,
           size = 2,
           conf.int.level = 0.99) +
...
# Shapiro-Wilks test'as PSAI kintamąjam
shapiro.test(homogeniai_duomenys$PSAI)
# Kolmogorov-Smirnov test'as PSAI kintamąjam
ks.test(
  homogeniai_duomenys$PSAI,
  "pnorm",
  mean = mean(homogeniai_duomenys$PSAI),
  sd = sd(homogeniai_duomenys$PSAI)
)
normalumas_psai <-
  ggqqplot(homogeniai_duomenys$PSAI,
           size = 2,
           conf.int.level = 0.99) +
  xlab("Teoriniai kvantiliai") +
  ylab("Imties kvantiliai")  +
...
testavimo_imtis <- sutvarkyti_duomenys %>%
  ...
  select(-numeris, -OS, -Death_other_causes)
