...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Pathologic_stage,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Lymph_nodes_status,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
      rename(Lymph_nodes_status  = LNI01),
    conf.int = 0.95
  )
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Surgical_margin_status,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
      rename(Surgical_margin_status  = R1),
    conf.int = 0.95
  )
...
fit <-
  survfit(
    Surv(Survival_years, CSS) ~ Metastatic_lymph_nodes,
    data = sutvarkyti_duomenys %>%
      mutate_if(is.factor, ~ as.numeric(as.character(.x))) %>%
      rename(Metastatic_lymph_nodes = GLpoRP_ISUP),
    conf.int = 0.95
  )
